...
#input_dir = r"data/6.10/CT_map/"
#xlsx_dir = r"data/6.10/cube/"
input_dir = r"D:/Project/NerveStitcher-master/dataset/sh/prep_data/OD/"
xlsx_dir = r"D:/Project/NerveStitcher-master/dataset/sh/prep_data/OD/"
if not os.path.exists(input_dir + '/result/'):
    os.makedirs(input_dir + '/result/')
data = pd.read_excel(xlsx_dir + "测试数据6.10.xlsx")
row = data.shape[0]
data1 = data.values
for i in range(row):
    img1 = data1[i][0]
    ...
    if x_move == "None":
        ...
        x_move = [int(x_move)]
        y_move = [int(y_move)]
        img1 = cv2.imread(input_dir + '%s' % img1)
        sum_x, sum_y, x, y = calculate_img_size(x_move, y_move)
        bg_img = create_img([834, 500], sum_x, sum_y)
        center_point, bg_img = combine_first_img(img1, bg_img, x, y)

        img2 = cv2.imread(input_dir + '%s' % img2)
        img2 = Image.fromarray(np.uint8(img2))
        bg_img = Image.fromarray(np.uint8(bg_img))
        center_point[0] += -np.int(x_move[0])
        center_point[1] += -np.int(y_move[0])
        bg_img.paste(img2, (center_point[0], center_point[1]))
        bg_img = np.asarray(bg_img)  # 得到拼接图像
        cv2.imwrite(input_dir + r'result/result%i.jpg' % (i+2), bg_img)
        x_move.clear()
        y_move.clear()
        print(i)
